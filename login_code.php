<?php 
	...
	if(isset($_POST['login'])) {
		...
		if(mysqli_num_rows($check)===1) {
				...
				$check2 = mysqli_query($conn, "SELECT * FROM household_head WHERE email='$email' AND password='$password'");
                if(mysqli_num_rows($check2)===1) {
					...
					if($row['email'] === $email && $row['password'] === $password) {
						...
						header('Location: Users/about_me.php?household_Id='.$household_Id.'');
