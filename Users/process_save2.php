<?php 
      ...
      if(mysqli_num_rows($fetch) > 0) {
        ...
        if(isset($_POST['ID'])) {
           ...
           $fetch = mysqli_query($conn, "SELECT * FROM documents WHERE household_Id='$id' AND (ID_2 = '' || Barangay_Certification = '' || Punong_Barangay = '' || Walang_Kinakasama = '' || Certificate_Ng_Anak = '' || certificate_choices = '' || Treasurer_Certification = '')");
            if(mysqli_num_rows($fetch) > 0) {
        ...
        // PWD APPLICATION
          if(isset($_POST['Certification_of_Disability'])) {
           ...
           $file = basename($_FILES["fileToUpload"]["name"]);
            ...
            $fetch = mysqli_query($conn, "SELECT * FROM documents_pwd WHERE household_Id='$id' AND (Certification_of_Disability = '' || Blood_Type = '' || Body_Picture = '' || ID_Picture = '' || Copy_Birth_Certificate = '')");
            if(mysqli_num_rows($fetch) > 0) {
          ...
          if(isset($_POST['Blood_Type'])) {
           ...
           $file = basename($_FILES["fileToUpload"]["name"]);
            ...
            $fetch = mysqli_query($conn, "SELECT * FROM documents_pwd WHERE household_Id='$id' AND (Certification_of_Disability = '' || Blood_Type = '' || Body_Picture = '' || ID_Picture = '' || Copy_Birth_Certificate = '')");
            // $fetch = mysqli_query($conn, "SELECT * FROM documents_pwd WHERE Blood_Type != '' AND household_Id='$id'");
            if(mysqli_num_rows($fetch) > 0) {
       ...
       // SENIOR APPLICATION
          if(isset($_POST['Birth_certificate'])) {
           ...
           $file = basename($_FILES["fileToUpload"]["name"]);
            ...
            $fetch = mysqli_query($conn, "SELECT * FROM documents_solo_parent WHERE household_Id='$id' AND (Birth_certificate = '' || Marriage_contract = '' || Valid_Id = '')");
            if(mysqli_num_rows($fetch) > 0) {
                ...
                header('Location: upload_senior_documents.php?household_Id='.$id.'');    
                  // if everything is ok, try to upload file
                      ...
                      if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                            ...
                            $udpate = mysqli_query($conn, "UPDATE documents_solo_parent SET Birth_certificate='$file' WHERE household_Id='$id'");
                            if($udpate) {
                              $_SESSION['success']  = "File has been uploaded.";
                             header('Location: upload_senior_documents.php?household_Id='.$id.'');        
                            } else {
                 ...
                 header('Location: upload_senior_documents.php?household_Id='.$id.''); 
                  // if everything is ok, try to upload file
                      ...
                      if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                            ...
                            // $udpate = mysqli_query($conn, "UPDATE documents_pwd SET Certification_of_Disability='$file' WHERE household_Id='$id'");
                             $save = mysqli_query($conn, "INSERT INTO documents_solo_parent (household_Id, Birth_certificate)  VALUES ('$id', '$file')");
                            if($save) {
                              $_SESSION['success']  = "File has been uploaded.";
                              header('Location: upload_senior_documents.php?household_Id='.$id.''); 
                            } else {
                ...
                header('Location: upload_senior_documents.php?household_Id='.$id.'');    
                  // if everything is ok, try to upload file
                      ...
                      if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                            ...
                            $udpate = mysqli_query($conn, "UPDATE documents_solo_parent SET Marriage_contract='$file' WHERE household_Id='$id'");
                            if($udpate) {
                              $_SESSION['success']  = "File has been uploaded.";
                             header('Location: upload_senior_documents.php?household_Id='.$id.'');        
                            } else {
                 ...
                 header('Location: upload_senior_documents.php?household_Id='.$id.''); 
                  // if everything is ok, try to upload file
                      ...
                      if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                            ...
                            // $udpate = mysqli_query($conn, "UPDATE documents_pwd SET Certification_of_Disability='$file' WHERE household_Id='$id'");
                             $save = mysqli_query($conn, "INSERT INTO documents_solo_parent (household_Id, Marriage_contract)  VALUES ('$id', '$file')");
                            if($save) {
                              $_SESSION['success']  = "File has been uploaded.";
                              header('Location: upload_senior_documents.php?household_Id='.$id.''); 
                            } else {
      ...
      // SENIOR APPLICATION
          if(isset($_POST['Valid_Id'])) {
           ...
           $file = basename($_FILES["fileToUpload"]["name"]);
            ...
            $fetch = mysqli_query($conn, "SELECT * FROM documents_solo_parent WHERE household_Id='$id' AND (Birth_certificate = '' || Marriage_contract = '' || Valid_Id = '' || Birth_certificate != '' || Marriage_contract != '' || Valid_Id != '')");
            if(mysqli_num_rows($fetch) > 0) {
                ...
                header('Location: upload_senior_documents.php?household_Id='.$id.'');    
                  // if everything is ok, try to upload file
                      ...
                      if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                            ...
                            $udpate = mysqli_query($conn, "UPDATE documents_solo_parent SET Valid_Id='$file' WHERE household_Id='$id'");
                            if($udpate) {
                              $_SESSION['success']  = "File has been uploaded.";
                             header('Location: upload_senior_documents.php?household_Id='.$id.'');        
                            } else {
                 ...
                 header('Location: upload_senior_documents.php?household_Id='.$id.''); 
                  // if everything is ok, try to upload file
                      ...
                      if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                            ...
                            // $udpate = mysqli_query($conn, "UPDATE documents_pwd SET Certification_of_Disability='$file' WHERE household_Id='$id'");
                             $save = mysqli_query($conn, "INSERT INTO documents_solo_parent (household_Id, Valid_Id)  VALUES ('$id', '$file')");
                            if($save) {
                              $_SESSION['success']  = "File has been uploaded.";
                              header('Location: upload_senior_documents.php?household_Id='.$id.''); 
                            } else {
